...
#define pr(...) std::println(__VA_ARGS__)

#ifdef LOCAL
#define dbgln(...) cout << __VA_ARGS__ << '\n'
...
void solve() {
  ...
  auto can = [&](ll x) {
    ...
    // NOTE: did == k instead of >= (can extend any mex array if has mex of x)
    // also, fucked up binary search with a[i] > x (WRONG, can still form mex, doesn't negate)
    return groups >= k;
  };
